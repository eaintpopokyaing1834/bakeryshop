<?php
// ─────────────────────────────────────────────────────────────────────────────
// auth/login.php — Combined Admin / Cashier staff login (glassmorphic UI)
// ─────────────────────────────────────────────────────────────────────────────
session_start();
require_once __DIR__ . '/../includes/lang.php';
require_once __DIR__ . '/../config/db.php';

// ── 1. Already logged-in guard ────────────────────────────────────────────────
if (isset($_SESSION['user_id'])) {
    $role = $_SESSION['role'] ?? '';
    if ($role === 'admin') {
        header('Location: /sweetheaven/admin/dashboard.php');
    } elseif ($role === 'cashier') {
        header('Location: /sweetheaven/cashier/pos.php');
    } else {
...
// ── 2. Database setup ─────────────────────────────────────────────────────────
$db = getDB();
try { $db->exec("ALTER TABLE users MODIFY COLUMN role ENUM('admin','customer','cashier') DEFAULT 'customer'"); } catch (PDOException $e) {}
try { $db->exec("ALTER TABLE users ADD COLUMN status ENUM('active','inactive') DEFAULT 'active' AFTER role"); } catch (PDOException $e) {}

// ── 3. Determine active tab (survives a failed POST) ──────────────────────────
$active_role = 'admin';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $active_role = $_POST['role'] ?? 'admin';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!in_array($role_req, ['admin', 'cashier'])) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            if (($user['status'] ?? 'active') === 'inactive') {
                ...
                $error = __('login_err_invalid'); // vague — don't reveal real role
                ...
                if ($remember) {
                    ...
                    try {
                        ...
                        $db->prepare("INSERT INTO remember_tokens (user_id, token_hash, expires_at) VALUES (?, ?, ?)")
                           ->execute([$user['id'], $hash, date('Y-m-d H:i:s', $expires)]);
                        setcookie('remember_staff', $token, [
                            'expires'  => $expires,
                            'path'     => '/sweetheaven',
                            'httponly' => true,
                            'samesite' => 'Lax',
...
<body class="min-h-screen overflow-hidden">
    ...
    <div class="fixed inset-0 z-0">
        <img
            src="/sweetheaven/images/cashier4cake.jpg"
            alt=""
    ...
    <div class="relative z-10 min-h-screen flex items-center justify-center p-4">
        ...
        <div class="w-full max-w-md
            ...
            <div class="px-8 pt-8 pb-4 flex flex-col items-center">
                <img src="/sweetheaven/images/9102671.png"
                     alt="Sweet Heaven"
                     class="h-14 w-auto mb-3 drop-shadow-lg">
                <h1 class="text-2xl font-bold text-white tracking-tight drop-shadow">Sweet Heaven</h1>
                <p class="text-white/60 text-md font-light mt-0.5">Staff Portal</p>
            </div>
            ...
            <div class="px-8 pt-2 pb-5">
                <div class="flex rounded-xl bg-white/10 border border-white/15 p-1 gap-1" id="roleTabs">
                    ...
                    <!-- Admin tab -->
                    <button type="button" id="tab-admin"
                    ...
                    <!-- Cashier tab -->
                    <button type="button" id="tab-cashier"
            ...
            <div class="px-8 pb-8">
                ...
                <form method="POST" action="" id="loginForm">
                    ...
                    <!-- Email -->
                    <div class="mb-4">
                        <label for="email" class="block text-xs font-semibold text-white/70 uppercase tracking-wider mb-1.5">
                            Email Address
                        </label>
                        <div class="relative">
                    ...
                    <div class="mb-4">
                        <div class="flex items-center justify-between mb-1.5">
                            <label for="password" class="block text-xs font-semibold text-white/70 uppercase tracking-wider">
                                Password
                            </label>
                        ...
                        <div class="relative">
                            ...
                            <button type="button" id="pwToggle"
                                ...
                                <svg id="eyeShow" class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                          d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                          d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                </svg>
                                <svg id="eyeHide" class="w-[18px] h-[18px] hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <!-- Submit button — colour changes with active role -->
                    <button type="submit" id="submitBtn"
                        ...
                        <span id="btnLabel"><?= $active_role === 'admin' ? 'Sign In as Admin' : 'Sign In as Cashier' ?></span>
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <script>
        ...
        function switchRole(role) {
            ...
            if (role === 'admin') {
                ...
                // Activate cashier tab
                tabCashier.className = tabCashier.className
                    .replace('text-white/60 hover:text-white hover:bg-white/10', '')
                    .replace('  ', ' ').trim()
                    + ' bg-amber-500 text-white shadow-lg shadow-amber-500/30';
                // Deactivate admin tab
                tabAdmin.className = tabAdmin.className
