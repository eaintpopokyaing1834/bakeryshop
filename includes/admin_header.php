...
$totalNotifications = (int) $db->query("SELECT COUNT(*) FROM notifications WHERE is_seen=0")->fetchColumn();
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title><?= $pageTitle ?? 'Admin' ?> — Sweet Heaven Admin</title>
    ...
    <style>
        * {
        ...
        .sidebar-link {
            @apply flex items-center gap-3 px-4 py-2.5 rounded-lg text-stone-500 hover:text-stone-800 hover:bg-stone-100 transition-all duration-200 text-sm font-medium;
        }

        .sidebar-link.active {
            @apply bg-rose-50 text-rose-600 font-semibold;
        }
...
<body class="bg-pink-200 min-h-screen flex">

    <!-- Sidebar -->
    <aside id="sidebar"
        class="w-64 min-h-screen bg-white border-r border-stone-200/80 flex flex-col fixed top-0 left-0 z-40 transition-transform duration-300">

        <!-- Brand -->
        <div class="p-5 border-b border-stone-100">
            <a href="/sweetheaven/admin/dashboard.php" class="flex items-center gap-3">
                <div class="w-9 h-9 bg-rose-50 rounded-lg flex items-center justify-center">
                    <img src="/sweetheaven/images/shoplogo.png" class="h-6 w-auto" alt="">
                </div>
                <div>
                    <p class="text-stone-800 font-bold text-sm leading-tight">Sweet Heaven</p>
                    <p class="text-stone-400 text-xs">Admin Panel</p>
                </div>
            </a>
        </div>
        ...
        <nav class="flex-1 p-4 space-y-1">
            <p class="text-stone-400 text-xs font-semibold uppercase tracking-wider px-4 mb-2">Main</p>
            ...
            <a href="/sweetheaven/admin/product.php"
                class="sidebar-link <?= $currentPage === 'product' ? 'active' : '' ?>">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
            ...
            <a href="/sweetheaven/admin/order.php" class="sidebar-link <?= $currentPage === 'order' ? 'active' : '' ?>">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                ...
                <?php if ($newOrdersCount > 0): ?>
                    <span
                        class="ml-auto bg-rose-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
                        <?= $newOrdersCount > 99 ? '99+' : $newOrdersCount ?>
                    </span>
                <?php endif; ?>
            </a>

            <a href="/sweetheaven/admin/user.php" class="sidebar-link <?= $currentPage === 'user' ? 'active' : '' ?>">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
            ...
            <div class="pt-4 border-t border-stone-100 mt-4">
                <p class="text-stone-400 text-xs font-semibold uppercase tracking-wider px-4 mb-2">Store</p>
                <a href="/sweetheaven/user/index.php" target="_blank" class="sidebar-link">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
        ...
        <div class="p-4 border-t border-stone-100">
            <div class="flex items-center gap-3 bg-stone-50 rounded-xl p-3">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-stone-700 text-sm font-semibold truncate">
                        <?= htmlspecialchars($_SESSION['name'] ?? 'Admin') ?>
                    </p>
                    <p class="text-stone-400 text-xs">Administrator</p>
                </div>
                <a href="/sweetheaven/auth/logout.php" class="text-stone-400 hover:text-rose-500 transition-colors"
                    title="Logout">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
    ...
    <div class="flex-1 ml-64 flex flex-col min-h-screen">
        ...
        <header
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-xl font-bold text-stone-800"><?= $pageTitle ?? 'Dashboard' ?></h1>
                    <p class="text-xs text-stone-400"><?= date('l, F j, Y') ?></p>
                </div>
            </div>
