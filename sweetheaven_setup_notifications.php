...
if (!$tableExists) {
    ...
    foreach ([
        ...
        try { $db->exec($fk); } catch (Exception $e) { /* ignore */ }
    }
} else {
    ...
    foreach ([
        ...
        try { $db->exec($sql); } catch (Exception $e) { /* already exists — fine */ }
    }
}
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
        ...
        <div class="space-y-1 font-mono text-sm">
            ...
            <div class="<?= $type === 'ok' ? 'text-green-700' : ($type === 'warn' ? 'text-amber-600' : 'text-gray-500') ?>">
                <?= $msg ?>
            </div>
            <?php endforeach; ?>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
        ...
        <p class="text-gray-500 text-sm mb-4">
            This injects a <b>"processing"</b> notification for every existing customer order so you can verify the bell works immediately.
        </p>
        <?php if ($testSent): ?>
        <div class="bg-green-50 border border-green-200 text-green-700 rounded-xl px-4 py-3 text-sm font-semibold mb-4">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
        ...
        <?php if (empty($rows)): ?>
        <p class="text-gray-400 text-sm text-center py-8">No notifications yet. Use the button above to send a test one.</p>
        <?php else: ?>
            ...
            <table class="w-full text-sm">
                ...
                <?php foreach ($rows as $r): ?>
                    <tr class="hover:bg-gray-50/50">
                        ...
                        <td class="px-3 py-2 font-mono text-gray-500"><?= $r['order_id'] ? '#'.str_pad($r['order_id'],4,'0',STR_PAD_LEFT) : '—' ?></td>
                        <td class="px-3 py-2">
                            <span class="bg-rose-50 text-rose-600 text-xs px-2 py-0.5 rounded-full font-semibold"><?= htmlspecialchars($r['type']) ?></span>
                        </td>
                        <td class="px-3 py-2 text-center">
                            <?= $r['is_seen'] ? '<span class="text-green-500">✔</span>' : '<span class="text-rose-500 font-bold">●</span>' ?>
                        </td>
                        <td class="px-3 py-2 text-gray-600 max-w-xs truncate"><?= htmlspecialchars($r['message'] ?? '') ?></td>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
        ...
        <ol class="space-y-3 text-sm text-gray-600">
            <li class="flex gap-3">
                <span class="w-6 h-6 bg-rose-500 text-white rounded-full flex items-center justify-center text-xs font-bold shrink-0">1</span>
                <span>Admin changes order status (e.g. <b>processing</b>, <b>shipped</b>, <b>delivered</b>) in <code class="bg-gray-100 px-1 rounded">admin/order.php</code></span>
            </li>
            <li class="flex gap-3">
                <span class="w-6 h-6 bg-rose-500 text-white rounded-full flex items-center justify-center text-xs font-bold shrink-0">2</span>
                <span>PHP inserts a row into <code class="bg-gray-100 px-1 rounded">notifications</code> with the customer's <b>user_id</b>, <b>order_id</b>, <b>type='order_status'</b>, and a friendly message</span>
            </li>
            <li class="flex gap-3">
                <span class="w-6 h-6 bg-rose-500 text-white rounded-full flex items-center justify-center text-xs font-bold shrink-0">3</span>
                <span>Customer's browser polls <code class="bg-gray-100 px-1 rounded">/api/user_notifications.php?action=count</code> every <b>30 seconds</b> — badge animates in if new ones appear</span>
            </li>
            <li class="flex gap-3">
                <span class="w-6 h-6 bg-rose-500 text-white rounded-full flex items-center justify-center text-xs font-bold shrink-0">4</span>
                <span>Customer clicks the 🔔 bell → dropdown loads notifications from <code class="bg-gray-100 px-1 rounded">/api/user_notifications.php?action=list</code></span>
            </li>
            <li class="flex gap-3">
                <span class="w-6 h-6 bg-rose-500 text-white rounded-full flex items-center justify-center text-xs font-bold shrink-0">5</span>
                <span>Clicking any notification → marks all as seen (<code class="bg-gray-100 px-1 rounded">is_seen=1</code>) and navigates to the orders tab</span>
            </li>
        </ol>
    </div>
