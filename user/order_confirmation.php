...
$order = $db->prepare("
    SELECT o.*, u.name AS customer_name, pm.payment_name, pm.acc_name, pm.acc_no, p.status AS pay_status
    FROM orders o
...
$order = $order->fetch();
if (!$order) { header('Location: /sweetheaven/user/index.php'); exit; }
...
<head>
    ...
    <style>
        * { font-family: 'Poppins', sans-serif; }
        ...
        .checkmark-path { stroke-dasharray: 50; stroke-dashoffset: 50; animation: checkmark 0.8s ease forwards 0.5s; }
...
<div class="max-w-2xl mx-auto px-6 py-16 text-center">
    ...
    <p class="text-gray-500 mb-2">Thank you, <strong><?= htmlspecialchars($order['customer_name']) ?></strong>!</p>
    <p class="text-gray-400 text-sm mb-8">Your order <span class="text-rose-500 font-bold">#<?= str_pad($order['id'], 4, '0', STR_PAD_LEFT) ?></span> has been received and is being processed.</p>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 text-left mb-8">
        <div class="grid sm:grid-cols-2 gap-4 mb-6 pb-6 border-b border-gray-100 text-sm">
            ...
            <div>
                ...
                <p class="font-bold text-gray-800"><?= date('M j, Y g:i A', strtotime($order['order_date'])) ?></p>
            </div>
            <div>
                <p class="text-gray-400 text-xs mb-1">Shipping</p>
                <p class="font-bold text-gray-800 capitalize"><?= $order['shipping_method'] ?> delivery</p>
            </div>
            <div>
                <p class="text-gray-400 text-xs mb-1">Status</p>
                <span class="inline-flex bg-amber-100 text-amber-700 text-xs font-bold px-3 py-1 rounded-full">Pending</span>
            </div>
        </div>
        ...
        <div class="space-y-3 mb-6 pb-6 border-b border-gray-100">
            ...
            <div class="flex justify-between items-center text-sm">
                <span class="text-gray-700"><?= htmlspecialchars($item['product_name']) ?> <span class="text-gray-400">x<?= $item['quantity'] ?></span></span>
                <span class="font-semibold text-gray-800"><?= number_format($item['price'] * $item['quantity']) ?> MMK</span>
            </div>
            <?php endforeach; ?>
        </div>
        ...
        <div class="flex justify-between items-center font-bold text-gray-800">
            ...
            <span class="text-xl text-rose-500"><?= number_format($order['total_amount']) ?> MMK</span>
        </div>
        ...
        <div class="mt-6 pt-6 border-t border-gray-100 bg-amber-50 rounded-2xl p-5">
            ...
            <p class="text-sm text-amber-700">Please transfer <strong><?= number_format($order['total_amount']) ?> MMK</strong> to:</p>
            <p class="text-sm font-bold text-amber-800 mt-1"><?= htmlspecialchars($order['payment_name']) ?></p>
            <?php if ($order['acc_name']): ?>
            <p class="text-sm text-amber-700">Account Name: <?= htmlspecialchars($order['acc_name']) ?></p>
            <p class="text-sm text-amber-700">Account No: <strong><?= htmlspecialchars($order['acc_no']) ?></strong></p>
            <?php endif; ?>
        </div>
        <?php endif; ?>
    </div>
