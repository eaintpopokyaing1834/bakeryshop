...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_product'])) {
    ...
    if (!empty($_FILES['images']['name'][0])) {
        ...
        $isPrimary = ($_POST['set_primary'] ?? 0) == 1;
        // If new product, first image is primary
        $existingCount = $db->prepare("SELECT COUNT(*) FROM product_images WHERE product_id=?");
        $existingCount->execute([$productId]);
...
<div class="flex gap-2 mb-6">
    <a href="?tab=products" class="px-6 py-2.5 rounded-xl font-semibold text-sm transition-colors
    ...
    <a href="?tab=categories" class="px-6 py-2.5 rounded-xl font-semibold text-sm transition-colors
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        <h3 class="font-bold text-gray-800">All Products <span class="text-gray-400 font-normal text-sm ml-2">(<?= count($products) ?>)</span></h3>
        <button onclick="openProductModal()" class="bg-rose-500 hover:bg-rose-600 text-white px-4 py-2 rounded-xl text-sm font-semibold transition-colors flex items-center gap-2">
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tr class="hover:bg-gray-50/50 transition-colors">
                <td class="px-6 py-4">
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-12 rounded-xl overflow-hidden bg-rose-50 shrink-0">
                            ...
                            <img src="/sweetheaven/<?= htmlspecialchars($p['primary_image']) ?>" class="w-full h-full object-cover" alt="">
                            <?php else: ?>
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400 line-clamp-1"><?= htmlspecialchars(substr($p['description'],0,60)) ?>...</p>
                        </div>
                    </div>
                </td>
                ...
                <td class="px-6 py-4">
                    <span class="text-xs font-bold px-2.5 py-1 rounded-full <?= $p['stock'] < 10 ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' ?>">
                        <?= $p['stock'] ?>
                    </span>
                </td>
                <td class="px-6 py-4">
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        <h3 class="font-bold text-gray-800">All Categories <span class="text-gray-400 font-normal text-sm ml-2">(<?= count($categories) ?>)</span></h3>
        <button onclick="openCategoryModal()" class="bg-rose-500 hover:bg-rose-600 text-white px-4 py-2 rounded-xl text-sm font-semibold transition-colors flex items-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
            Add Category
        </button>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tr class="hover:bg-gray-50/50 transition-colors">
                ...
                <td class="px-6 py-4 text-sm text-gray-500"><?= htmlspecialchars(substr($cat['description'] ?? '', 0, 80)) ?></td>
                <td class="px-6 py-4">
                    <div class="flex items-center gap-2">
        ...
        <form method="POST" enctype="multipart/form-data" action="?tab=products" class="p-6 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="price" id="productPrice" required min="0" step="100" placeholder="5000"
                        class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-rose-300 text-sm">
                </div>
                <div>
                ...
                <div class="col-span-2">
                    ...
                    <p class="text-xs text-gray-400 mt-1">First image uploaded will be set as primary.</p>
                </div>
            </div>
            <div class="flex gap-3 pt-2">
                <button type="submit" class="flex-1 bg-rose-500 hover:bg-rose-600 text-white font-semibold py-3 rounded-xl transition-colors">Save Product</button>
                <button type="button" onclick="closeModal('productModal')" class="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-600 font-semibold rounded-xl transition-colors">Cancel</button>
            </div>
        </form>
    </div>
</div>
...
<div id="categoryModal" class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-sm w-full max-w-md">
        ...
        <form method="POST" action="?tab=categories" class="p-6 space-y-4">
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="flex-1 bg-rose-500 hover:bg-rose-600 text-white font-semibold py-3 rounded-xl transition-colors">Save Category</button>
                <button type="button" onclick="closeModal('categoryModal')" class="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-600 font-semibold rounded-xl transition-colors">Cancel</button>
            </div>
        </form>
    </div>
</div>
